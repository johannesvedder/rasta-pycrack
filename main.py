...
def retrieve_safety_code():
    ...
    try:
        pcap_file = RawPcapReader(args['filepath'])
    except FileNotFoundError:
        raise FileNotFoundError('Could not find the pcap file')

    print("Analyzing pcap file for RaSTA traffic and saving data to rasta.p")
    ...
    for (data, pkt_metadata,) in pcap_file:
        ...
        data = {"pdu": pdu, "code": safety_code}
        pickle.dump(data, open("rasta.p", "wb"))

        return pdu, safety_code
...
if __name__ == "__main__":
    ...
    try:
        ...
        pdu_bytes, safe_code = retrieve_safety_code()

    pdu_hash = calculate_safety_code(pdu_bytes, IV)

    if pdu_hash == safe_code:
        print("Woop woop hashes are matching!")
    else:
        print("ERROR: Hashes not equal")
